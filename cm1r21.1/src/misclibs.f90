...
!----------------------------------------------------------------------
      ...
      DO j=1,nj
        ...
        do i=1,ni
          budj(j)=budj(j)+(dum2d5(i,j)-dum2d2(i,j))
        enddo
...
!!!                     abs(khv(i,j,k))*dtdz*mf(i,j,k)*mf(i,j,k) )
      ...
      if( cm1setup.eq.2 .and. idopbl .and. horizturb.eq.0 )then
        fhmax=0.0
        imaxh=0
        jmaxh=0
        kmaxh=0
      endif
      if( cm1setup.eq.2 .and. idopbl .and. ipbl.ne.2 )then
        fvmax=0.0
        imaxv=0
        jmaxv=0
        kmaxv=0

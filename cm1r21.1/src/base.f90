...
!-----------------------------------------------------------------------------
      ...
      subroutine base(zh,mh,rmh,c1,c2,zf,mf,rho0s,pi0s,prs0s,rth0s,         &
                      ...
                      qc0,qi0,rr0,rf0,rrf0,t0  ,rh0 ,rst0,                  &
...
!  BLG soundings
      ...
      IF(isnd.eq.1300)THEN
        call inversionfog_base(zh,pi0,prs0,th0,thl0,qv0,qc0,rst0,t0)
      ELSEIF(isnd.eq.1301)THEN
        call mixedcldlayer_base(zh,pi0,prs0,th0,thl0,qv0,qc0,rst0,t0)
      ELSEIF(isnd.eq.1302)THEN
        call mixedunsatlayer_base(zh,pi0,prs0,th0,thl0,qv0,qc0,rst0,t0)
      ELSEIF(isnd.eq.1303)THEN
        call mixeddrylayer_base(zh,pi0,prs0,th0,thl0,qv0,qc0,rst0,t0)
      ELSEIF(isnd.eq.1304)THEN
        call marginalcld_base(zh,pi0,prs0,th0,thl0,qv0,qc0,rst0,t0)
      ELSEIF(isnd.eq.1305)THEN
        call cldoverclear_base(zh,pi0,prs0,th0,thl0,qv0,qc0,rst0,t0)

!-----------------------------------------------------------------------
!  isnd = 1
!  Dry adiabatic base state

      ELSEIF(isnd.eq.1)THEN
      ...
      if(imoist.eq.1)then
        do k=1,nsnd
          if(qvsnd(k).lt.1.0e-12)then
            if(dowr) write(outfile,*) '  Qv is too small.  Setting rh to 0.01%.  k,zsnd=',k,zsnd(k)
            qvsnd(k)=0.0001*rslf(psnd(k),thsnd(k)*pisnd(k))
          endif
...
!   RCEMIP  (Wing et al. 2018, GMD)
      ...
      ELSEIF(isnd.eq.21)THEN
        ...
        tsfc = tsk0           ! set air temperature equal to SST 
...
!  BLG winds
      ...
      IF(iwnd.eq.1300)then
        call linearshear_wind(zh,u0,v0)

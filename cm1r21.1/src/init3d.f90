...
!-----------------------------------------------------------------------------
      ...
      use bc_module
      use module_mp_nssl_2mom, only: ccn, lccn, irenuc
      use poiss_module
      use parcel_module , only : getparcelzs
      use init3d_blg_module
      ...
      real, dimension(ib:ie,jb:je,kb:ke) :: pi0,prs0,rho0,thv0,th0,thl0,rth0,qv0
      real, dimension(ib:ie,jb:je,kb:ke) :: qc0,qi0,rr0,rf0,rrf0,rst0
...
! initialize CCN concentrations as constant mixing ratios througout the domain
      ...
      do j=jbm,jem
      do i=ibm,iem
       IF ( irenuc < 5 ) THEN ! number of unactivated ccn
       qa (i,j,k,lccn-1) = ccn/1.225
       q3d(i,j,k,lccn-1) = ccn/1.225
       ELSE ! treat as number of activated ccn
       qa (i,j,k,lccn-1) = 0.0
       q3d(i,j,k,lccn-1) = 0.0
       ENDIF
      enddo
      enddo
      enddo
...
!---- This is here to ensure that certain idealized cases work ----
    ...
    IF( ptype.ge.1 )THEN
      ...
      IF( (isnd.eq.4 .or. isnd.eq.9 .or. isnd.eq.10 .or. isnd.eq.11 .or. isnd.eq.15 &
            .or. (isnd.ge.1300 .and. isnd.le.1399)) )THEN
...
!  BLG perturbations
      ...
      IF(iinit.eq.1300)THEN
        call inversionwave(xh,xf,yh,yf,zh,zf,pi0,prs0,th0,thl0,&
                              qv0,qc0,rst0,u0,v0,ua,va,wa,tha,qa)
      ELSEIF(iinit.eq.1302)THEN
        call linearshear_mode(xh,xf,yh,yf,zh,zf,pi0,prs0,th0,thl0,&
                              qv0,qc0,rst0,u0,v0,ua,va,wa,tha,qa)
      ELSEIF(iinit.eq.1305)THEN
        call cldoverclear_wave(xh,xf,yh,yf,zh,zf,pi0,prs0,th0,thl0,&
                               qv0,qc0,rst0,u0,v0,ua,va,wa,tha,qa)
...
!!!        yref  =  -100000.0    ! y location of TC center (m)
      ...
      ! do not change anything below here !
        ...
        ELSE
          ...
          if( myid.eq.0 ) print *,'  rrmax,nref = ',rrmax,nref
      ...
      do nn = 1 , 9
        ...
        IF( doit )THEN
          ...
          if( rr.lt.rrmax )then
            ...
            if(nn.eq.1)then
              tha(i,j,k) = thref(nref,k)
              ppi(i,j,k) = piref(nref,k)
              qa(i,j,k,nqv) = qvref(nref,k)
            endif
          endif
          enddo
          enddo
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      real, intent(inout), dimension(ibb:ieb,jbb:jeb,kbb:keb) :: upten,vpten
      real, intent(inout), dimension(ibcu:iecu,jbcu:jecu,kbcu:kecu) :: ucuten,vcuten
...
!  Get stuff
  ...
  IF( ( .not. restarted ) .or.( interp_on_restart ) )THEN
...
!$omp private(i,j,k)
    ...
    ELSE
      ...
      if(myid.eq.0) print *
      if(myid.eq.0) print *,'  getting prs,rho '
      if(myid.eq.0) print *
...
!  cm1r18:  get total mass of dry air at t=0
      ...
      IF( ( .not. restarted ) .or. ( interp_on_restart ) )THEN
        ...
        if(myid.eq.0) print *
        if(myid.eq.0) print *,'  Getting mass1 '
        if(myid.eq.0) print *
...
!------------------------------------------------------------------
        ...
        tavgcode:  &
        IF( dotimeavg .and. ( (.not. restarted) .or. interp_on_restart ) )THEN
          if(myid.eq.0) print *
          if(myid.eq.0) print *,'  initializing timeavg fields '
          if(myid.eq.0) print *
          if(utav.ge.1)then
            do k=1,keta
            do j=0,nj+1
            do i=0,ni+2
              timavg(i,j,k,utav) = u3d(i,j,k)
            enddo
            enddo
            enddo
          endif
          ...
          if(qtav.ge.1)then
            ...
            do j=0,nj+1
            do i=0,ni+1
              timavg(i,j,k,qtav) = q3d(i,j,k,nqv)
            enddo
            enddo
            enddo
          endif
